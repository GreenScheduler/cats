...
def csv_loader(filename):
    ...
    data = [[datetime.datetime.strptime(x[0], '%Y-%m-%dT%H:%M:%SZ'), float(x[1])] for x in data]
    return data
...
def get_lowest_carbon_intensity(data, method='simple', size=None):
    ...
    if method == 'windowed':
        ...
        if size > len(data):
            raise ValueError('Windowed method timespan cannot be greater than the cached timespan')
